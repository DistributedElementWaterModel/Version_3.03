...
void HbvAquifer::WaterBalance(int timeStep, DateTime datetime, double upLandAccumulatedLowerDischarge, double upLandAccumulatedUpperDischarge, int dayofyear)
{
    GetVegetation()->WaterBalance(timeStep, datetime, missingData, missingData, dayofyear, GetSnow()->GetSnowCoverFraction());
    GetSnow()->WaterBalance(timeStep, datetime, GetVegetation()->GetThroughFall(), GetVegetation()->GetTemperature());
    GetHBV()->WaterBalance(timeStep, datetime, GetSnow()->GetWaterOutput(), GetVegetation()->GetTemperature(), GetSnow()->GetSnowCoverFraction(), GetVegetation()->GetDryPeriod(), upLandAccumulatedLowerDischarge, upLandAccumulatedUpperDischarge, GetVegetation()->GetPET_PM());
}
...
double HbvAquifer::GetPET() const
{
	double pet = 0.0;
	if (GetNextHbvAquifer())
		pet = pet + GetNextHbvAquifer()->GetPET();
	    pet = pet + (GetVegetation()->GetPET_PM()*GetAreaFraction() / 100.0);
	return pet;
}
